...
def learn(env,
    ...
    # Replay buffer #
    replay_buffer = ReplayBuffer(500000, frame_history_len)
    ...
    for i in range(outer_loop):
        ...
        print('prob prod')
        print(episode_storage['episode_prob_traj_prod'])
        ...
        # Update Target Network (so that it does not have q-values based on old reward)
        ...
        phis = np.hstack((phis,phi[:,np.newaxis]))
        crystals = np.append(crystals,episode_storage['episode_crystals_captured'])
        asteroids = np.append(asteroids,episode_storage['episode_asteroid_collisions'])
        aliens = np.append(aliens,episode_storage['episode_alien_collisions'])
        np.savetxt(savedir+'/phis.txt',phis)
        np.savetxt(savedir+'/crystals.txt',crystals)
        np.savetxt(savedir+'/asteroids.txt',asteroids)
        np.savetxt(savedir+'/aliens.txt',aliens)
