...
def cq_model(img_in, num_actions, scope, reuse=False):
    ...
    # Probably can re-read in tensorflow variables.
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        with tf.variable_scope("convnet"):
            ...
            # Chris' architecture
            #out = layers.convolution2d(out, num_outputs=8, kernel_size=4, stride=2, activation_fn=tf.nn.relu)
            #out = layers.convolution2d(out, num_outputs=16, kernel_size=2, stride=1, activation_fn=tf.nn.relu)
            #out = layers.convolution2d(out, num_outputs=64, kernel_size=2, stride=1, activation_fn=tf.nn.relu)

            # Test architecture for speed:
